...
func NewRootCmd() *cobra.Command {
	...
	root.Flags().BoolVar(&location, "location", true,
		"telemetry only: write the clip's GPS position into the output's container metadata (the \"location\" tag and Apple's \"com.apple.quicktime.location.ISO6709\"). On by default. Pass --location=false to leave it out: the tag is read by YouTube, Photos, Immich and QuickTime, so a run that starts at your front door otherwise ships your home address in the file. This governs only the tag videofx WRITES -- metadata is carried over from the source wholesale, so a position the camera already recorded survives regardless (strip it with ffmpeg -map_metadata -1 or exiftool), and it does not affect telemetry-hud's burned-in course map, which is in the pixels. Note --effect telemetry-hud implies --effect telemetry, so this applies to a HUD burn as well")
