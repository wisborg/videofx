...
- `--warp-model` — gocv-stabilizer only: the per-frame motion model. `similarity` (default) fits one 4-DOF transform (pan/rotate/scale) per frame. `homography` is **EXPERIMENTAL and currently not recommended**: it additionally fits an 8-DOF homography per frame to correct the perspective/shear a similarity can't represent (rolling-shutter skew, wide-lens parallax). A single global homography *registers* very-shaken frames ~42% better than a similarity, but as a per-frame stabilizer the 8-DOF fit's higher variance **injected more jitter than it removed** on the test footage (measured residual `12.15` vs similarity's `10.81`), i.e. it made the result worse. It's kept opt-in as scaffolding for a future variance-controlled (mesh/bundled-paths) implementation, which is what a tool like Premiere's Warp Stabilizer uses to get the benefit without the variance. Baked into a `--sidecar`'s analysis, so change `--warp-model` and `--sidecar` together (or delete the sidecar) to re-analyze.
