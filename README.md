...
- `--warp-model` — gocv-stabilizer only: the per-frame motion model.
  ...
  - `mesh` (**EXPERIMENTAL**) adds a MeshFlow-style spatially-varying correction on top: it median-votes each frame's local feature motions onto a grid of vertices (tunable via `--mesh-grid`), smooths each vertex's path, and warps the frame with the resulting mesh — targeting the rolling-shutter/parallax jitter a single global transform leaves behind. The **median voting is the variance control** the `homography` model lacked, and because it corrects only the *residual* beyond the similarity, it's a no-op on rigid/gentle footage (no regression there). Measured on the very-shaken test clip it **beats similarity** — median residual `9.42` vs `10.81`, closing ~⅓ of the gap to Premiere's `6.49` — with no visible warping. Still being tuned (grid size, smoothing); not yet the default.
...
- `--mesh-grid` — gocv-stabilizer only, `--warp-model mesh`: the mesh grid size, in cells across the frame width (the vertical count is derived to keep cells roughly square). `0` = default `16`. A finer grid corrects more localized motion but is noisier per vertex, which shows up as a wigglier warp — coarser reads cleaner. Best value still being tuned empirically. Baked into a `--sidecar`'s analysis (change it and `--sidecar` together).
- `--mesh-strength` — gocv-stabilizer only, `--warp-model mesh`: the mesh correction gain, `0.0`–`1.0`. A spatially-varying warp inherently trades some picture distortion (a per-frame bend/swim) for stabilization, so **lower this to reduce the distortion** at a little less shake removal; `1.0` is full strength, `0` disables the mesh (falls back to similarity). `-1` (default) uses the built-in gentle default of `0.5`. Unlike `--mesh-grid`, this is applied at **render time**, so it can be swept against a cached `--sidecar` without re-analyzing.
